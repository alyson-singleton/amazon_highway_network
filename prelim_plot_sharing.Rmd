...
shapefiles_to_remove <- c(57,220,34,31,104,57,22,102,43,188,29,37,28,27,26,174,96,112,8,9,4,10,113,63,106,260,84,1,22,146,32,40,105,36,190,221,103,83,98,99,97)
...
proj_gt130 <- which(osm_amazon_roads_ref_br_extras$first_ref %in% as.character(osm_ref_freq$ref[which(osm_ref_freq$frequency>130)]))
osm_amazon_roads_ref_br_extras_proj_gt130 <- osm_amazon_roads_ref_br_extras[proj_gt130,]

#colors for n=16
library(pals)
pal.bands(tol, n=12)
# 
proj_gt130_map <- ggplot() +
  geom_sf(data = brazil_amazon_municipalities, fill='white', color='lightgrey', size=.15, show.legend = FALSE) +
  geom_sf(data = brazil_amazon, fill=NA, color='black', size=.15, show.legend = FALSE) +
  geom_sf(data = osm_amazon_roads_ref_br_extras_proj_gt130, aes(geometry = geometry, color=first_ref), linewidth=0.8, show.legend = "line") +
#  scale_color_manual(name="OSM (fclass)", values=c('#6A359CFF','#FFB04FFF','#CD1076FF', '#6497B1FF'),
#                     labels=c('Trunk','Primary','Secondary', 'Tertiary')) +
  theme_minimal() +
  scale_colour_manual(values=c(unname(tol())[1],"blue",unname(tol())[2:6], "gold", unname(tol())[7:11], "red", unname(tol())[12], "pink")) +
