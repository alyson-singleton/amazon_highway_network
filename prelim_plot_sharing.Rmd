...
## Figure 2: Paving status change from 2013 to 2024

Data posted by Brazil's National Department of Transportation Infrastructure. Very detailed data with shapefiles going back to 2013. Temporal resolution is quarterly back to 2016, with one entry for 2015 and 2013 (none for 2014).
...
## Figure 3: Paving time series from 2013 to 2024

Same data as above colored by year(ish) of paving.

```{r}
DNIT_all_years <- st_read("~/Desktop/doctorate/ch3 amazon network/data/DNIT/DNIT_processed.shp")
...
DNIT_time_map <- ggplot() +
  ...
  scale_color_manual(name="Year Paved", values=c('#648FFF', '#785EF0', '#ffc500', '#FF7900','#DC267F'),
...
DNIT_future_map <- ggplot() +
  ...
  scale_color_manual(name="Road Status", values=c('#648FFF', '#ffc500', '#DC267F'),
                     labels=c('Not paved', 'Paved', 'Planned')) +
  theme_minimal() +
  no_axis +
  theme(legend.text=element_text(size=12),
        legend.title=element_text(size=14),
